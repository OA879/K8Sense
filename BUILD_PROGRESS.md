...
| 2a. Rule library (63 of 63 planned) | ✅ Done | 8 categories; the last 5 land via the API server's /metrics + /readyz and the VPA CRD (below) |
...
- `checks/metrics.go` — the final 5 checks, which read data the plain
  object-listing API can't give, all still via `Discovery().RESTClient().AbsPath`
  (no dynamic client or extra config): CP-001 (API server /readyz reports a
  failing subsystem), CP-006 (etcd slow — apiserver-observed
  `etcd_request_duration_seconds`), CP-007 (API server 5xx rate from
  `apiserver_request_total`), CERT-005 (client certs near expiry from
  `apiserver_client_certificate_expiration_seconds`), RES-006 (VPA
  recommendation diverges >1.5x from the target Deployment's requests). Each
  degrades gracefully: a forbidden `/metrics` (common on managed control
  planes) is *skipped*; an absent metric or VPA CRD is *not applicable*.
- **63 rules across 8 categories** in `rules/*.yaml` — the full planned
  catalogue. Verified against a live cluster: a scan runs all 63, and the
  metric-driven checks fire correctly on real data (e.g. CP-006 flagged 1.26%
  of etcd requests over 1s on a cold-disk kind cluster) or cleanly skip where
  not applicable. Parsing/threshold logic is unit-tested in `metrics_test.go`.
- **Hardening:** the `/metrics` parser must be created via
  `expfmt.NewTextParser` — a bare `TextParser{}` has an unset name-validation
  scheme and *panics* on the first metric line (caught by the live smoke, would
  have crashed a real scan). The scanner now also wraps every check in
  `runCheckSafely`, turning any check panic into a skipped-check rather than a
  failed scan.
...
Buildable now (not blocked on third parties):
1. **Container image end-to-end** — the Dockerfile + rules bundling are correct
   and unit-guarded, but a full image build + in-cluster deploy still needs a
   machine with memory headroom (this dev VM is shared with other workloads).
2. **Lens-style resource forms** — surface/polish the inherited Headlamp
   create/edit flows (RBAC, PVC, StorageClass, ServiceAccount).

Blocked on your accounts/infra:
3. **Signed installers** — Apple + Windows EV certs. Unsigned installers already
   build via `.github/workflows/release.yml` on a version tag.
4. **SSO / team sharing** — a real IdP + Postgres.
5. **Commercial distribution** — Stripe + a domain for the licence-purchase
   flow (the Ed25519 licence *validator* is already built).
