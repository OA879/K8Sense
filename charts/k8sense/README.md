...
K8sense is an easy-to-use and extensible Kubernetes web UI that provides:
...
Add the K8sense repository and install the chart:
...
Access K8sense:
```console
$ kubectl port-forward -n kube-system svc/my-k8sense 8080:80
```
...
```console
$ helm install my-k8sense k8sense/k8sense \
...
$ helm install my-k8sense k8sense/k8sense \
  ...
  --set ingress.hosts[0].host=k8sense.example.com \
...
```console
$ helm upgrade my-k8sense k8sense/k8sense \
  --namespace kube-system \
  --reuse-values \
...
> **Warning**
> Cluster Inventory support in K8sense is alpha/experimental and disabled by
...
`image` volumes; it is not for K8sense UI plugins.
...
| image.repository | string | `"k8sense-k8s/k8sense"` | Container image name |
...
| Key                | Type   | Default               | Description                                                               |
|--------------------|--------|-----------------------|---------------------------------------------------------------------------|
| config.inCluster   | bool   | `true`                | Run K8sense in-cluster                                                   |
| config.baseURL     | string | `""`                  | Base URL path for K8sense UI                                             |
...
| config.pluginsDir  | string | `"/k8sense/plugins"` | Directory to load K8sense plugins from                                   |
...
| config.clusterInventory.labelSelector | string | `"!k8sense.dev/ignore"` | Kubernetes label selector used to filter experimental/alpha ClusterProfile resources |
| config.clusterInventory.rootReconcileInterval | string | `""` | Override the experimental/alpha Cluster Inventory root reconcile interval. Empty uses the K8sense default |
| config.clusterInventory.noCRDCacheTTL | string | `""` | Override the experimental/alpha Cluster Inventory no-CRD cache TTL. Empty uses the K8sense default |
| config.extraArgs   | array  | `[]`                  | Additional arguments for K8sense server                                  |
...
mounted and readable in the K8sense container. The default path is provided by
...
config:
  oidc:
    ...
    meUserInfoURL: "https://k8sense.example.com/oauth2/userinfo"
```
...
and adds the K8sense Cluster Inventory flags automatically.
...
`plugins[]` is for Cluster Inventory access provider binaries, not K8sense UI
plugins. Each entry renders as a Kubernetes `image` volume and is mounted
read-only into the K8sense container. If an access provider `execConfig.command`
...
- An `emptyDir` volume named `k8sense-tmp` is created and mounted at `/tmp` in the main container.
- If `pluginsManager` is enabled and its effective security context (own or inherited) also sets `readOnlyRootFilesystem: true`, a separate `emptyDir` volume named `k8sense-plugins-tmp` is created and mounted at `/tmp` in the plugin manager container.
...
```yaml
# Provide your own k8sense-tmp volume (e.g. to set a size limit).
...
`backend.service.{name,port}` to override the default K8sense Service /
    ...
    - host: k8sense.example.com
      paths:
        ...
        - path: /extra
          ...
          backend:
            service:
              # name is optional; defaults to the K8sense Service.
...
For users who prefer Gateway API over classic Ingress resources, K8sense supports HTTPRoute configuration.
...
```yaml
config:
  tlsCertPath: "/k8sense-cert/tls.crt"
  tlsKeyPath: "/k8sense-cert/tls.key"
...
topologySpreadConstraints:
  - maxSkew: 1
    ...
    labelSelector:
      matchLabels:
        app.kubernetes.io/name: k8sense
        custom-label: value
```
...
| version         | string  | `latest`          | K8sense plugin package version to install                                                |
...
We welcome contributions to the K8sense Helm chart! To contribute:
...
3. Run Helm chart template tests to ensure your changes don't break existing functionality:
   ...
   ```
   This command executes the script at `charts/k8sense/tests/test.sh` to validate Helm chart templates against expected templates.
...
For more details, refer to our [contributing guidelines](https://github.com/kubernetes-sigs/k8sense/blob/main/CONTRIBUTING.md).

## Links

- [GitHub Repository](https://github.com/kubernetes-sigs/k8sense)
- [Documentation](https://k8sense.dev/)
- [Maintainers](https://github.com/kubernetes-sigs/k8sense/blob/main/OWNERS_ALIASES)
